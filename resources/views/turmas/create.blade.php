...
<div class="container mx-auto py-12 px-4">
    <div class="flex justify-between items-center mb-10">
        <h1 class="text-3xl font-light tracking-tight">Adicionar Turma</h1>
            ...
            <form action="{{ route('turmas.store') }}" method="POST" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="numero" id="numero" value="{{ old('numero') }}" required class="apple-input h-11 w-full px-4 py-2 text-sm">
                    </div>
                    
                    <div class="space-y-2">
                        <label for="semestre" class="text-sm font-medium text-muted-foreground">Semestre</label>
                        <input type="text" name="semestre" id="semestre" value="{{ old('semestre') }}" required class="apple-input h-11 w-full px-4 py-2 text-sm">
                    </div>
                    
                    <div class="space-y-2">
                        <label for="periodo" class="text-sm font-medium text-muted-foreground">Período</label>
                        <input type="text" name="periodo" id="periodo" value="{{ old('periodo') }}" required class="apple-input h-11 w-full px-4 py-2 text-sm">
                    </div>
                    
                    <div class="space-y-2">
                        <label for="professor" class="text-sm font-medium text-muted-foreground">Professor</label>
                        <input type="text" name="professor" id="professor" value="{{ old('professor') }}" required class="apple-input h-11 w-full px-4 py-2 text-sm">
                    </div>
                    
                    <div class="space-y-2">
                        <label for="sala" class="text-sm font-medium text-muted-foreground">Sala</label>
                        <input type="text" name="sala" id="sala" value="{{ old('sala') }}" required class="apple-input h-11 w-full px-4 py-2 text-sm">
                    </div>
                    
                    <div class="space-y-2">
                        <label for="horario" class="text-sm font-medium text-muted-foreground">Horário</label>
                        <input type="text" name="horario" id="horario" value="{{ old('horario') }}" required class="apple-input h-11 w-full px-4 py-2 text-sm">
                    </div>
                </div>
